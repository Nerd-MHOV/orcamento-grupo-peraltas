...
export function Row(props: {
  ...
  async function handleSendBudget(
    rd_id: string | undefined,
    budget_id: string,
    budget: BudgetsContentProps[]
  ) {
    if (!rd_id) return;
    alert("Enviando!");
  }
  ...
  async function handleFavorite(id: string) {
    api.budget.favorite(id).then((response) => {
      reloadRows();
    });
  }
  ...
  return (
    <React.Fragment>
      <TableRow sx={{ "& > *": { borderBottom: "unset" } }}>
        ...
        <TableCell>
          <BudgetNameField name={row.name} id={row.id} reload={reloadRows} />
        </TableCell>
        ...
        <TableCell>
          <StatusBudget status={row.status} />
        </TableCell>
        <TableCell>
          <IconButton
            aria-label="expand row"
          ...
          <Collapse in={open} timeout="auto" unmountOnExit>
            <>
              ...
              >
                <Button
                  onClick={() => {
                    handleSendBudget(row.rd_client, row.id, row.budgets);
                  }}
                >
                  Enviar para Cliente
                </Button>
