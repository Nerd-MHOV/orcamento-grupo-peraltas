...
export async function rdSaveProcessCorp(budget: CorporateBodyResponseBudget) {
  ...
  const adt = budget.rooms.reduce(
    (accumulator, currentValue) => accumulator + currentValue.adt,
    0
  );
  ...
  await apiAddProd(dateIn, dateOut, budget.rowsValues.total.total, idClient);
  await api.rd.changeStage(
    idClient,
...
async function apiAddProd(
  ...
  const api = useApi();
  try {
    await api.tariff.orm(dateIn, dateOut).then((tariff_id) => {
      // pipe.addFile();
